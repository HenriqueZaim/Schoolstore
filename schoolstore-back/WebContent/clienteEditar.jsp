...
<body>
	...
	<header style="background: rgba(0, 0, 0, 0.8);">
		<nav
			class="navbar navbar-expand-lg navbar-dark bg-transparent shadow-none scrolling-navbar">
			<div class="container">
				...
				<form id="clienteFormulario" method="post" action="app">
						...
						<div class="col-md-10">
							...
							<div class="form-row">
								<div class="col mb-3 md-form">
									<label for="txtNome" class="active">Nome Completo*</label> <input
					...
					<div class="row justify-content-center">
						<div class="col-md-10">
							<div class="row mt-5 align-items-center">
								...
								<a href="#" class="btn btn-link btn-rounded p-2 mb-2"
									data-toggle="modal" data-target="#modalEndereco"> <i
									class="fas fa-plus "></i>
								</a>
							</div>
							<div class="accordion md-accordion" id="accordionEndereco"
								role="tablist" aria-multiselectable="true">
								<c:forEach var="endereco" items="${cliente.getEnderecos()}">
									<div class="card">
										<div class="card-header" role="tab" id="${endereco.getId()}">
											<h5 class="mb-0">
												${endereco.getNome()} <a data-toggle="collapse"
													data-parent="#accordionEndereco"
													href="#${endereco.getCep()}-${endereco.getId()}"
													aria-expanded="false"
													aria-controls="${endereco.getCep()}-${endereco.getId()}">
													<i class="fas fa-angle-down rotate-icon"></i>
												</a>
												<button
													onclick="$(this).parent().parent().parent().remove()"
													class="btn btn-link p-0 m-0 float-right mr-5">
													<i class="fas fa-times text-danger"></i>
												</button>
											</h5>
										</div>
										<div id="${endereco.getCep()}-${endereco.getId()}"
											class="collapse show" role="tabpanel"
											aria-labelledby="${endereco.getId()}"
											data-parent="#accordionEndereco">
											<div class="card-body">
												<ul>
										...
										<input type="hidden" name="txtEnderecoId"
											value="${endereco.getId()}"> <input type="hidden"
											name="txtCep" value="${endereco.getCep()}"> <input
											type="hidden" name="txtBairro"
											value="${endereco.getBairro()}"> <input type="hidden"
											name="txtLogradouro" value="${endereco.getLogradouro()}">
										<input type="hidden" name="txtCidadeId"
											value="${endereco.getCidade().getId()}"> <input
											type="hidden" name="txtReferencia"
											value="${endereco.getReferencia()}"> <input
											type="hidden" name="txtComplemento"
											value="${endereco.getComplemento()}"> <input
											type="hidden" name="txtNumero"
											value="${endereco.getNumero()}"> <input type="hidden"
											name="txtFavoritoEndereco" value="${endereco.isFavorito()}">
										<input type="hidden" name="txtNomeEndereco"
											value="${endereco.getNome() }">
									</div>
								</c:forEach>
							</div>
						</div>
					...
					<div class="row justify-content-center">
						<div class="col-md-10">
							<div class="row mt-5 align-items-center">
								...
								<a href="" class="btn btn-link btn-rounded p-2 mb-2"
									data-toggle="modal" data-target="#modalCartao"> <i
									class="fas fa-plus "></i>
								</a>
							</div>
							<div class="accordion md-accordion" id="accordionCartao"
					...
					<div class="row mt-5 text-center">
						<div class="col-md-12">
							...
							<button class="btn btn-warning btn-lg btn-rounded" type="submit">Editar
								Cadastro</button>
						</div>
					</div>
				</form>
				...
				<div class="modal-body mx-3">
					<div class="form-row">
						<div class="col-md-4 mb-3 md-form">
							<label for="txtCepModal">CEP*</label> <input type="text"
								class="form-control" id="txtCepModal" required maxlength="9">
						</div>
						<div class="col-md-8 mb-3 md-form">
							<label for="txtLogradouroModal">Logradouro*</label> <input
								type="text" class="form-control" id="txtLogradouroModal"
								required maxlength="100">
						</div>
					</div>
					<div class="form-row">
						<div class="col-md-6 mb-3 md-form">
							<select class="mdb-select" id="txtEstadoModal" required
								searchable="Selecione..">
								...
								<c:forEach var="estado" items="${estados}">
									<c:forEach var="cidade" items="${estado.getCidades()}">
										<option value="${cidade.getId()}"
											class="${cidade.getEstado().getId()}">${ cidade.getNome() }</option>
									</c:forEach>
								</c:forEach>
							</select>
						</div>
					...
					<div class="form-row">
						<div class="col-md-4 mb-3 md-form">
							<label for="txtNumeroModal">Número*</label> <input type="number"
								min="1" max="9999" maxlength="4" class="form-control"
								id="txtNumeroModal" required>
						</div>
						<div class="col-md-4 mb-3 md-form">
							<label for="txtBairroModal">Bairro*</label> <input type="text"
								class="form-control" id="txtBairroModal" required
								maxlength="100">
						</div>
						<div class="col-md-4 mb-3 md-form">
							<label for="txtComplementoModal">Complemento</label> <input
					...
					<div class="form-row">
						<div class="col mb-3 md-form">
							<label for="txtReferenciaModal">Referência</label> <input
								type="text" class="form-control" id="txtReferenciaModal"
								required maxlength="100">
						</div>
					</div>
					<div class="form-row">
						<div class="col mb-3 md-form">
							<label for="txtNomeEnderecoModal">Identificação de
								Endereço*</label> <input type="text" class="form-control"
								id="txtNomeEnderecoModal" required maxlength="100">
						</div>
					</div>
					<div class="form-row">
						<div class="col mb-3 md-form">
							<input class="form-check-input" type="checkbox" value="false"
								id="txtFavoritoModal"> <label class="form-check-label"
								for="txtFavoritoModal"> Marcar como favorito </label>
						</div>
					</div>
				</div>
	...
	<div class="modal fade" id="modalCartao" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header text-center">
				...
				<div class="modal-body mx-3">
					<div class="form-row">
						<div class="col mb-3 md-form">
							<label for="txtNomeImpressoModal">Nome Impresso*</label> <input
								type="text" class="form-control" id="txtNomeImpressoModal"
								required maxlength="100">
						</div>
					</div>
					<div class="form-row">
						<div class="col-md-8 mb-3 md-form">
							<label for="txtNumeroCartaoModal">Número*</label> <input
								type="text" maxlength="16" class="form-control"
								id="txtNumeroCartaoModal" required>
						</div>
						<div class="col-md-4 mb-3 md-form">
							<label for="txtCodigoModal">Código*</label> <input type="text"
								class="form-control" id="txtCodigoModal" required maxlength="3">
						</div>
					</div>
					<div class="form-row">
						<div class="col mb-3 md-form">
							<input class="form-check-input" type="checkbox" value="false"
								id="txtFavoritoCartaoModal"> <label
